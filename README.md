...
The four bundled demos have also been rebuilt with the current router from their saved designs and placements,
without model calls: all 11 power rails pass width checks, with zero ETCH/KiCad errors, warnings or unconnected
items. Replay events, downloads and renders come from the same rebuild. See
[the offline refresh workflow](CONTRIBUTING.md#refreshing-the-four-demos-without-api-credits) for future updates.
